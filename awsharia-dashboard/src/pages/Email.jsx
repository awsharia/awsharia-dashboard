...
export default function Email({ kitData }) {
  ...
  // Build mock growth from total (real growth needs separate API calls)
  const growth = Array.from({length:6},(_,i)=>{
    const months = ['Dec','Jan','Feb','Mar','Apr','May'];
    const ratio = 0.57 + i*0.086;
    return { month: months[i], amount: Math.round(d.totalSubscribers * ratio) };
  });
  ...
  return (
    <>
      ...
      <KpiGrid>
        <KpiCard label="Total subscribers" value={d.totalSubscribers.toLocaleString()} sub="all statuses" />
        <KpiCard label="Active subscribers" value={d.activeSubscribers.toLocaleString()} sub="confirmed + active" />
        <KpiCard label="Last broadcast" value={lastBc ? Math.round(lastBc.openRate*100)+'%' : '—'} sub={lastBc ? 'open rate — '+fmtDate(lastBc.sentAt) : 'open rate'} />
        ...
        <Panel>
          ...
          <Table headers={['Subject','Sent','Recipients','Opens','Clicks']}>
            {d.broadcasts.map((b,i)=>(
              <Tr key={i}>
                <Td style={{ maxWidth:200, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{b.subject}</Td>
      ...
      <Panel style={{ marginBottom:20 }}>
        <PanelHead title="Subscriber growth" meta={`+${(d.totalSubscribers - growth[0].amount).toLocaleString()} over 6 months`} />
