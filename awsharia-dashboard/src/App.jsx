...
export default function App() {
  ...
  const hasAnyKey = () => !!(
    process.env.REACT_APP_KIT_KEY ||
    process.env.REACT_APP_GSHEETS_KEY ||
    process.env.REACT_APP_HAS_STRIPE === 'true'
  );
  ...
  const loadLive = useCallback(async () => {
    ...
    // Start with demo data — replace piece by piece as keys become available
    ...
    try {
      // Stripe (serverless — no browser key needed)
      try {
      ...
      if (hasSheets()) {
        ...
        if (resolvedPayments === DEMO_PAYMENTS && process.env.REACT_APP_PAYMENTS_SHEET_ID) {
          ...
          } catch { /* payments sheet not set up yet */ }
        }
      }
      ...
      if (process.env.REACT_APP_KIT_KEY) {
        try {
          setKitData(await fetchKit(process.env.REACT_APP_KIT_KEY));
          anyLiveData = true;
  ...
  useEffect(() => {
    // Always load — never show config screen automatically
    loadLive();
  }, [loadLive]);
      ...
      <aside style={{
        ...
        <nav style={{ padding: '16px 12px', flex: 1 }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              border: 'none', fontSize: 13, fontWeight: 400, transition: 'all 0.15s',
            }}>{t}</button>
          ))}
        </nav>
        <div style={{ padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <button onClick={refresh} style={{
            ...
            borderRadius: 4, fontSize: 11, fontFamily: 'var(--font-mono)', marginBottom: 6,
          }}>{loading ? 'Syncing…' : 'Refresh'}</button>
          <button onClick={() => setShowConfig(true)} style={{
            width: '100%', padding: '7px 0', background: 'transparent',
            border: 'none', color: 'rgba(255,255,255,0.3)', fontSize: 11, fontFamily: 'var(--font-mono)',
          }}>Settings</button>
        </div>
      </aside>
